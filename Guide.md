...
- 公式的插入
    ...
    - 多行公式：
        ...
        $$
        ```
        - 说明：将要写的公式放在`\begin{aligned}`和`\begin{aligned}`语句之间，使用`\\`换行，`&`来对齐公式
...
- 内容梳理概念
    - 定义：对于指定材料及相关资料经过理解消化后，对内容进行划分，分层，总结，整理后基于无序列表，采用各种文本样式，按照各项标准和规范，制作出的一份包含了材料即相关资料所有知识点及相关内容的，易读易理解的文件
    - 目的：充分学习指定材料及相关资料的知识，然后公开分享学习成果

- 一分内容梳理文件的基本要素，通常包含以下部分：
    - 枝干：拿到一份源文件首先需要根据文件所讲述的内容划分枝干
        ...
        - 小节（可选）
            - 使用二级标题表示
            - 样例`# §<章节序数>.<小节序数> <小节名称>`
    - 内容：除去枝干部分的所有文字，图片，公式，代码等都属于内容，所有的内容都写在无序列表下，其中可以穿插使用有序列表，表格，图片，超链接等丰富文本样式和内容，一般地我们给出如下示例样式：
    ___
    >- 知识点
    >    - 概念
    >        - 子概念
    ...
    >    - ......
    ___
    *以上样板仅供参考，在实际生产中需要负责人自行根据材料判断，设计出更清晰，更易读，更易理解的内容样式*
- 生产步骤
    ...
    3. 理解并掌握材料内容后，开始进行知识梳理的内容生产，要求遵循各项标准规范，参考优秀的内容梳理样例尽可能地以更加易读易理解的方式表现材料内容
    4. 完成生产后，仔细检查做出的内容梳理，对于其中翻译不准确的，不易理解的地方进行改进，纠正各种错误，优化文案排版，对内容梳理的各个方面做出改善
    5. 完成生产
- 质量评判标准
    - 合格：
    ...
    - 良好：
        ...
        5. 内容基本没有错误，语法基本没有错误，专有名词旁有补充俄语，文案排版基本遵守规范
    - 优秀：
        1. 达到良好的标准
        2. 通过查询其他相关资料，对材料中未提及的相关内容进行了补充，丰富知识体系
        3. 内容完全没有错误，语法完全没有错误
        4. 正确的插入空格，使用标点符号，使用大小写、缩写等，文案的整体排版严格遵守规范