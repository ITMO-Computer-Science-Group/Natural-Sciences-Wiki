...
- 三角函数简化公式可将任意角度的三角函数变为锐角的三角函数，即 $\alpha\in(0,\frac{\pi}{2})$
  ...
  - 简化公式表:
      ...
      | $\tg{\phi}$ | $\ctg{\alpha}$ | $-\ctg{\alpha}$ | $-\tg{\alpha}$ | $\tg{\alpha}$ | $\ctg{\alpha}$ | $-\tg{\alpha}$ | $-\tg{\alpha}$ |
      | $\tg{\phi}$ | $\tg{\alpha}$ | $-\tg{\alpha}$ | $-\ctg{\alpha}$ | $\ctg{\alpha}$ | $\tg{\alpha}$ | $-\tg{\alpha}$ | $-\ctg{\alpha}$ |
...
- 基本公式：
    ...
    - $\ctg\alpha=\frac{1}{\tg\alpha}=\frac{\cos\alpha}{\sin\alpha},且\alpha\neq k\pi,k\in Z$
